const inventors = [
  ...
  */
  ...
  veryOldInventors = inventors.filter(
    (inventor) => inventor.year >= 1500 && inventors.year <= 1600
  );
  // Check your work:
  ...
  */
  ...
  // Complete the exercise in the space below:
  //.map method applies a function to each element in the array
  firstLast = people.map(names => {
    let [last,first] = names.split(','); //splits the array into last and first
    return `${first} ${last}` //then returns the re
  });
  // Check your work:
